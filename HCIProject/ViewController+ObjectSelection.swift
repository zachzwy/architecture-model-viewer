...
extension ViewController: VirtualObjectSelectionViewControllerDelegate {
    ...
    func placeVirtualObject(_ virtualObject: VirtualObject) {
        ...
        self.generalSectionButton.isHidden = false
        
        savedPosition = virtualObject.position
        
        
        virtualObject.geometry?.firstMaterial?.diffuse.contents = UIColor.white
        
        updateQueue.async {
            let subAdd = self.sceneView.scene.rootNode.childNode(withName: "sub", recursively: true)!
            subAdd.addChildNode(virtualObject)
        ...
        virtualObjectLoader.loadVirtualObject(object, loadedHandler: { [unowned self] loadedObject in
                self.sceneView.prepare([object], completionHandler: { _ in
                    DispatchQueue.main.async {
                        ...
                        loadedObject.scale = SCNVector3Make(0.001, 0.001, 0.001)
                        loadedObject.isHidden = true
                        self.loadedObjects.append(loadedObject)
                    }
                })
            })
